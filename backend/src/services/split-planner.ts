 ...
 * タスク分割プランナー（MVP）
 *
 * 大きな task.md / goal.md を、独立して完走できる小さい light run 群へ
 * 分割する案を AI（read-only 実行）に出させる。案の承認と run 作成は
 * 人間が UI で行う — 提案は AI、決定は人間、の役割分離を維持する。
 */
...
function buildSplitPrompt(taskContent: string): string {
  return (
    'You are a planning assistant for the APSF workflow.\n' +
...
export function parseSplitProposals(raw: string): SplitProposal[] {
  ...
  return arr.map((p, i) => {
    ...
    // write-phase と同じ本物のルールで検証する（未チェックのチェックボックス等は数えない）
    if (PhaseDetector.countMeaningfulLines(task) <= 3) {
      throw new Error(
        `Proposal ${i + 1} ('${name}'): task body is too thin ` +
...
/**
 * 分割案を生成する（read-only AI 実行 — ファイルは一切変更させない）。
 */
export async function proposeSplit(
  runDir: string,
  provider: 'claude' | 'codex',
  workdir: string
): Promise<{ proposals: SplitProposal[]; raw: string }> {
  const taskPath = ['task.md', 'goal.md']
    .map((f) => path.join(runDir, f))
    .find((p) => fs.existsSync(p));
  if (!taskPath) throw new Error('task.md / goal.md not found — nothing to split.');
  const taskContent = fs.readFileSync(taskPath, 'utf-8');
  const prompt = buildSplitPrompt(taskContent);
  const timeoutMs = defaultExecTimeoutMs();

  // テスト用オーバーライド（native-executor と同じ流儀）
  ...
  if (override) {
    ...
    res = await spawnPrompt(
      'claude',
      ['-p', '--output-format', 'text', '--no-session-persistence', '--disable-slash-commands', '--permission-mode', 'dontAsk'],
      prompt,
      workdir,
      timeoutMs
    );
  ...
  if (res.code !== 0) {
    throw new Error(
      `split planner ${provider} exited with code ${res.code}: ${(res.stderr || res.stdout).slice(0, 300)}`
    );
  }
  const proposals = parseSplitProposals(res.stdout);
  // 監査用に案を run フォルダへ保存
  fs.writeFileSync(
    path.join(runDir, 'split_proposal.json'),
    JSON.stringify({ generated_at: new Date().toISOString(), provider, proposals }, null, 2),
    'utf-8'
  );
