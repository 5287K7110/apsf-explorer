...
function git(args: string[], cwd: string, timeoutMs = 15_000): Promise<{
  code: number | null;
  stdout: string;
  stderr: string;
}> {
...
export interface WorkdirGitStatus {
  isRepo: boolean;
  branch?: string;
  /** 変更のあるファイル（staged + unstaged + untracked） */
  dirtyFiles?: string[];
}

export async function workdirGitStatus(workdir: string): Promise<WorkdirGitStatus> {
...
export interface WorkdirGitDiff {
  isRepo: boolean;
  branch?: string;
  /** git diff --stat 相当の要約 */
  stat: string;
  /** unified diff（大きすぎる場合は打ち切り） */
  diff: string;
  truncated: boolean;
  untracked: string[];
}

export async function workdirGitDiff(workdir: string): Promise<WorkdirGitDiff> {
...
export function dirtySummary(status: WorkdirGitStatus): string | null {
  if (!status.isRepo || !status.dirtyFiles || status.dirtyFiles.length === 0) return null;
