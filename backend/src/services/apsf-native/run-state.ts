...
export const PHASE_OWNER: Record<string, string> = {
  TASK_NEEDED: 'Human',
  GOAL_NEEDED: 'Human',
  SETUP_NEEDED: 'Human',
  PLAN_NEEDED: 'Planner',
...
export function saveRunState(runDir: string, state: RunState): void {
  atomicWrite(
    path.join(runDir, STATE_FILENAME),
...
export function writeTransitionOutcome(
  runDir: string,
  opts: { fromPhase: string; toPhase: string; actor: string }
): void {
...
export function bootstrapRunState(
  runDir: string,
  opts: { phase: ApsfPhase; runType: 'light' | 'heavy' }
): void {
...
export function appendSessionEvent(
  ...
  fs.appendFileSync(
    path.join(runDir, 'session_events.jsonl'),
