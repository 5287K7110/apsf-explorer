/**
 * フェーズプロンプト組み立て（apsf act --print-prompt の TS ネイティブ移植）
 *
 * 参照仕様: ai-problem-solving-framework
 *   src/apsf/legacy/prompts/renderer.py（テンプレート文字列は忠実に一致させる）
 *   src/apsf/legacy/orchestration/act_service.py _read_context_files / _build_prompt
 */
...
/** renderer.py render_build_prompt */
...
/** renderer.py render_review_prompt */
...
/** act_service._read_context_files の忠実移植 */
function readContextFiles(info: PhaseInfo, runDir: string): Record<string, string> {
  const context: Record<string, string> = {};
  const filesToRead = [...info.filesToRead];
  if (
    !filesToRead.includes('execution-assignment.md') &&
...
export function buildPhasePrompt(runDir: string, frameworkRoot: string): {
  ...
  switch (info.phase) {
    ...
    case 'BUILD_NEEDED': {
      // Light run fallback: plan.md がなければ task.md
