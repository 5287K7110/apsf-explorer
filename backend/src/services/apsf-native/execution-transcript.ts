...
export interface TranscriptEvent {
  ts: number;
  type: string; // progress | complete | error | start
  data?: Record<string, unknown>;
}

export interface TranscriptMeta {
  file: string;
  startedAt: string; // ISO（ファイル名から復元）
  sizeBytes: number;
}

/** 新しいトランスクリプトのファイル名（20260708T123456-789Z-ab12cd.jsonl） */
function newTranscriptName(): string {
...
export class TranscriptWriter {
  ...
  constructor(runDir: string, initial: Record<string, unknown> = {}) {
    ...
    try {
      ...
      fs.writeFileSync(
        filePath,
        JSON.stringify({ ts: Date.now(), type: 'start', data: initial }) + '\n',
        'utf-8'
      );
    } catch {
  ...
  append(type: string, data?: Record<string, unknown>): void {
    ...
    try {
      fs.appendFileSync(
        this.filePath,
        JSON.stringify({ ts: Date.now(), type, data } satisfies TranscriptEvent) + '\n',
        'utf-8'
      );
    } catch { /* best-effort */ }
  }
...
export function listTranscripts(runDir: string): TranscriptMeta[] {
  const dir = path.join(runDir, EXECUTIONS_DIR);
 ...
 */
export function readTranscript(runDir: string, filename: string): TranscriptEvent[] | null {
