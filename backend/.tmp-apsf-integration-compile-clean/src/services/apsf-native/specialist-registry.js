...
export const PTYPE_TO_SPECIALIST = {
...
export const CTYPE_TO_SPECIALIST = {
...
function normalizeSpecialistCode(raw) {
    const m = (raw || '').match(SPECIALIST_CODE_RE);
    return m ? m[1].toUpperCase() : '';
}
const CONFIRMED_SECTION_HEADER = '## Confirmed Specialist';
export function extractPrimarySpecialistCode(text, prefix) {
    ...
    for (const line of lines) {
        ...
        if (inConfirmed && line.startsWith('## '))
            break;
...
function hasExplicitGenericSpecialist(text, prefix) {
    const marker = `${prefix}-TYPE`;
    for (const line of (text || '').split(/\r?\n/)) {
        const upper = line.toUpperCase();
        if (!upper.includes(marker))
            continue;
        if (upper.includes('NONE') || upper.includes('GENERIC'))
            return true;
    }
    return false;
}
export function extractSection(markdown, sectionName) {
    ...
    for (let i = 0; i < matches.length; i++) {
        if (matches[i][1].trim().toLowerCase() !== target)
            continue;
...
function extractFirstMatchingSection(markdown, names) {
...
function selectionSections(markdown) {
    return {
...
function scoreGoalAgainstSections(goalText, sections) {
    ...
    const score = scopeHits.length * 3 +
        evalHits.length * 2 +
        useHits.length * 3 +
        nearbyHits.length -
        outHits.length * 3 -
    ...
    const reasons = [];
    if (scopeHits.length)
        reasons.push(`scope hits: ${scopeHits.slice(0, 4).join(', ')}`);
    if (evalHits.length)
        reasons.push(`evaluation hits: ${evalHits.slice(0, 4).join(', ')}`);
    if (useHits.length)
        reasons.push(`use hits: ${useHits.slice(0, 4).join(', ')}`);
    if (nearbyHits.length)
        reasons.push(`nearby hits: ${nearbyHits.slice(0, 4).join(', ')}`);
    if (outHits.length)
        reasons.push(`out-of-scope hits: ${outHits.slice(0, 4).join(', ')}`);
    if (avoidHits.length)
        reasons.push(`avoid hits: ${avoidHits.slice(0, 4).join(', ')}`);
...
function criticHeuristicAdjustment(code, goalText) {
    ...
    if (code === 'C-10') {
        let bonus = 0;
        if (refactorHits.length)
            bonus += Math.min(refactorHits.length, 4) * 4;
        if (['split', 'extract', 'decompose', 'refactor'].some((t) => lowered.includes(t)) &&
...
function scoreSpecialist(goalText, sections, code) {
...
function loadSpecialistContent(code, frameworkRoot, mapping) {
...
function resolveSpecialist(goalText, assignmentText, frameworkRoot, prefix, mapping, explicitLabel) {
...
export function resolvePlannerSpecialist(goalText, assignmentText, frameworkRoot) {
    return resolveSpecialist(goalText, assignmentText, frameworkRoot, 'P', PTYPE_TO_SPECIALIST, 'explicit Primary P-TYPE');
}
export function resolveCriticSpecialist(goalText, assignmentText, frameworkRoot) {
