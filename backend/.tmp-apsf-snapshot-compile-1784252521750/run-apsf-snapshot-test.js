...
import { writePhase, sanitizePhaseInput, parseReviewJudgeAdvisory, } from './src/services/apsf-native/write-phase.js';
import { bootstrapRunState } from './src/services/apsf-native/run-state.js';
...
function checkSnapshot(name, actual, label) {
...
function testStartRunScaffold() {
    ...
    try {
        ...
        for (const [fname, template] of Object.entries(LIGHT_RUN_FILES)) {
            checkSnapshot(`scaffold-${fname}`, normalizeContent(fs.readFileSync(path.join(runDir, fname), 'utf-8')), `${fname} matches frozen template`);
        }
...
function testPhaseDetection() {
    ...
    try {
        ...
        for (const { name, phase, runType, setup } of phases) {
            ...
            bootstrapRunState(dir, { phase: phase, runType: runType });
            setup(dir);
            ...
            if (detected.humanOwned) {
                check(Boolean(detected.fileToWrite) && detected.fileToWrite !== '(none)', `${name} → human-owned phase has actionable fileToWrite (${detected.fileToWrite})`);
            }
        }
    }
    finally {
...
function testWritePhaseTransitions() {
    ...
    try {
        ...
        startRun(root, RUN, { light: true, taxonomy: 'work' });
        const runDir = path.join(root, 'runs', 'work', RUN);
        const inputs = { TASK_MD, BUILD_MD, REVIEW_MD, IMPROVE_MD };
        // 遷移列をスナップショットと比較
        const transitionLog = [];
        ...
        checkSnapshot('judge-advisory.json', JSON.stringify(stableAdvisory, null, 2), 'judge_advisory.json structure');
...
// ── 4. sanitize / reject ─────────────────────────────────────────────
function testSanitizeAndReject() {
    console.log('\n── sanitize / reject ──');
    const stripped = sanitizePhaseInput('[APSF] starting\n[Step 1/2] processing\n# Build\n\nReal content.\n');
...
function testJudgeAdvisory() {
    ...
    let threw = false;
    try {
        parseReviewJudgeAdvisory('# Review\n\nNo advisory block.\n');
    }
    catch {
        threw = true;
    }
